...
├── mosaic/                 # Mosaic library with AF3 modifications
│   └── src/mosaic/
│       ├── models/
│       │   └── af3.py      # ← AlphaFold 3 wrapper (main contribution)
│       ├── losses/
│       │   └── af3.py      # AF3Output helper (pLDDT, PAE, ipTM accessors)
│       ├── optimizers.py   # simplex_APGM
│       └── ...
└── requirements.txt
```
...
Download from [DeepMind's official repository](https://github.com/google-deepmind/alphafold3) and place the weight file (`af3.bin` or `af3.bin.zst`) in a local directory (e.g., `~/.alphafold3/model/`).
