...
from mosaic.losses.structure_prediction import IPTMLoss, AF3IPTMLoss, AbstractStructureOutput
...
class AlphaFold3(StructurePredictionModel):
    ...
    def __init__(
        ...
        # Load weights
        weights_file = model_dir / "af3.bin"
        if not weights_file.exists():
            raise FileNotFoundError(
                f"AF3 weights not found: {weights_file}\n"
                f"Expected af3.bin (decompressed from af3.bin.zst) in {model_dir}"
            )
        print(f"Loading AF3 weights from {weights_file} ({weights_file.stat().st_size/1e9:.2f} GB)...")
        haiku_params = af3_params.get_model_haiku_params(model_dir)
        if haiku_params is None:
            raise RuntimeError(f"af3_params.get_model_haiku_params returned None for {model_dir}")
        print(f"AF3 weights loaded ✓  ({sum(x.size for x in jax.tree_util.tree_leaves(haiku_params)):,} parameters)")
        ...
        # Minimize diffusion cost for gradient optimization.
        # Our losses (distogram, pLDDT, PAE/ipTM) come from the trunk + confidence head
        # and do NOT use diffusion structure samples. Reducing diffusion from the default
        # 5 samples × 200 steps to 1×1 cuts ~1000x memory in the backward pass.
        cfg.heads.diffusion.eval.num_samples = diffusion_num_samples
        cfg.heads.diffusion.eval.steps = diffusion_num_steps
        # Enable gradient checkpointing (remat) so JAX recomputes intermediate
        # activations rather than storing all 48 Evoformer layers at once.
        # block_remat=True with remat_block_size=8 gives ~8x peak memory reduction,
        # making gradient-based binder optimization feasible.
        cfg.evoformer.pairformer.block_remat = True
        cfg.heads.diffusion.transformer.block_remat = True

        # Haiku-transform the forward function
        def _forward(batch):
        ...
        # Transfer params to GPU/device as JAX arrays so equinox filter_jit can trace them
    ...
    def binder_features(self, binder_length: int, chains: list[TargetChain]):
        """
        Build features for a binder of given length prepended to target chains.
        Binder is chain A; target chains are B, C, ...
        """
        binder_chain = TargetChain(
            sequence="A" * binder_length,
            use_msa=False,
        )
        all_chains = [binder_chain] + list(chains)
        fi = _build_folding_input(all_chains, rng_seed=0)
        feats = _featurise(fi, self._ccd)
        return feats, None  # writer=None (AF3 has its own structure writer)
    ...
    def build_loss(
        ...
        *,
        ...
        # Convert numeric numpy arrays → JAX arrays for .at[...] ops in set_binder_sequence.
        # Object-dtype arrays (AF3 Structure objects) are left as-is.
        # features is marked static in AF3Loss so equinox won't trace the object arrays.
        def _to_jax(v):
    ...
    # ------------------------------------------------------------------ #
    # Model output (for evaluation / visualization)
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Predict (for end-to-end inference)
    # ------------------------------------------------------------------ #

    def _run(self, PSSM, features, key):
        # features must already have object-dtype arrays stripped (done in predict/model_output).
        # Uses _predict_jit (plain jax.jit) not _apply_fn — avoids equinox filter_jit which
        # can misclassify AF3 result dict entries as static, leaking tracers out of JIT scope.
        feats = set_binder_sequence(PSSM, features) if PSSM is not None else features
        result = self._predict_jit(self._params, key, feats)
        # result is now a materialized JAX pytree (concrete arrays, no tracers).
        seq = PSSM if PSSM is not None else jnp.zeros((0, 20))
        output = AF3Output(batch=feats, result=result)
        # Use AF3IPTMLoss which prefers tmscore_adjusted_pae_interface (fully differentiable)
        # over reconstructed pae_logits
        iptm = -AF3IPTMLoss()(seq, output, key=jax.random.key(0))[0]
        return result, feats, iptm, output
    ...
    def predict(
        ...
        *,
        ...
        # Strip object-dtype arrays; convert numeric numpy → JAX (needed for set_binder_sequence).
        # Keep original `features` for get_predicted_structure which needs numpy arrays.
        features_jit = {
            k: (jnp.array(v) if isinstance(v, np.ndarray) and v.dtype.kind != 'O' else v)
            for k, v in features.items()
            if not (isinstance(v, np.ndarray) and v.dtype.kind == 'O')
        }
        result, feats, iptm, output = self._run(PSSM, features_jit, key)

        # Build gemmi structure from AF3 predicted structure.
        # get_predicted_structure / feat_batch expect numpy arrays, not JAX arrays.
        ...
        # Convert alphafold3.Structure → gemmi.Structure
        import gemmi
        import io
