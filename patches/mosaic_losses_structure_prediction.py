...
class AF3IPTMLoss(LossTerm):
    """
    ipTM loss using AF3's native tmscore_adjusted_pae_interface output.

    Prefers this over IPTMLoss for AF3 because:
    - tmscore_adjusted_pae_interface is computed inside AF3 from softmax(pae_logits_true)
      and is directly differentiable (no logit reconstruction needed).
    - IPTMLoss reconstructs pae_logits via a Gaussian approximation of full_pae;
      this class bypasses that approximation entirely.

    Falls back to IPTMLoss if the key is not in the output (e.g. Boltz-2).
    """
    ...
    def __call__(
        ...
        # Try native AF3 interface TM score
        if hasattr(output, 'tmscore_adjusted_pae_interface'):
            adjusted_pae = output.tmscore_adjusted_pae_interface  # (N, N) or None
        else:
            adjusted_pae = None

        if adjusted_pae is not None:
            binder_len = sequence.shape[0]
            N = output.full_sequence.shape[0]
            asym_id = jnp.concatenate(
                (jnp.zeros(binder_len), jnp.ones(N - binder_len))
            ...
            # Average TM score over binder-target pairs, then take max over binder rows
            normed = interface_mask / (1e-8 + interface_mask.sum(axis=-1, keepdims=True))
            per_row = jnp.sum(adjusted_pae * normed, axis=-1)  # (N,)
            iptm = per_row[:binder_len].max()
            return -iptm, {"iptm": iptm}

        # Fallback: use reconstructed pae_logits (differentiable Gaussian approx)
        return IPTMLoss()(sequence, output, key)
